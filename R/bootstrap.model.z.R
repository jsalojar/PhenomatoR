...
bootstrap.model.z<-function(dataset,
                            ...
                            mc.cores = 1L) {
  #trim data
  trim.data<-dataset[,c(phenotype, covariant1, covariant2, randomeffect)]
  trim.data[,phenotype]<-as.numeric(trim.data[,phenotype])
  trim.data<-trim.data[stats::complete.cases(trim.data[,phenotype]),]
  ...
  if (is.null(n) == TRUE) {
    ...
    for (i in 1:iterations) {
      list.iterations[[i]] <- plyr::ddply(.data = trim.data,
                                          .variables = variables,
                                          .fun = function(x){x[sample(1:nrow(x),
                                                                      size = n,
                                                                      replace = TRUE),]},
                                          .parallel = TRUE)
    }
  ...
  if (is.null(covariant2) == TRUE && is.null(covariant2.control) == TRUE) {
    z.stats<-extract.glht.z(list.glht = list.glht, z.adjust = z.adjust, mc.cores = mc.cores)
    z.stats[,covariant1]<-levels(trim.data[,covariant1])[-1]
    z.stats$phenotype<-phenotype
  }
  else {
    z.stats1<-extract.glht.z(list.glht = list.glht1, z.adjust = z.adjust, mc.cores = mc.cores)
    ...
    z.stats2<-extract.glht.z(list.glht = list.glht2, z.adjust = z.adjust, mc.cores = mc.cores)
